...
formSignUp?.addEventListener("submit", async (event) => {
    ...
    const userNameInput = formSignUp.querySelector('input[name="name"]');
    const userEmailInput = formSignUp.querySelector('input[name="email"]');
    const userPasswordInput = formSignUp.querySelector('input[name="password"]');
    const userPasswordConfirmInput = formSignUp.querySelector('input[name="confirmPassword"]');
    const userPfp = document.querySelector("#file-upload");
    ...
    function checkUserName(name) {
        ...
        if (!checkedName) {
            if (name.length < 2 || name.length > 16) {
                ...
                addInputWarningMessage('Name either uses empty spaces or special characters that aren\'t underscore "_"');
            }
        }
        return checkedName;
    }
    function checkUserEmail(email) {
        ...
        if (/^[._]$/.test(localPart[0]) ||
            /^[._]$/.test(localPart[localPart.length - 1])) {
            addInputWarningMessage("First part of the email can't start or end with dot or underscore");
        }
        else if (localPart.length < 6 || localPart.length > 30) {
            addInputWarningMessage("First part of the email is not between 6 to 30 characters");
        }
        else if (!domainRegex.test(domainPart)) {
            addInputWarningMessage('Email must have either gmail, outlook, hotmail, email and end with ".com"');
        }
        else if (!localPartRegex.test(localPart)) {
            addInputWarningMessage("Email cant contain special characters that aren't non-consecutive dots or underscores");
        }
    ...
    function checkUserPassword(password, confirmedPassword) {
        const checkedPassword = password.length >= 8 && password.length <= 20 && /^\S+$/.test(password);
        if (!checkedPassword) {
            addInputWarningMessage("Password must be between 8 and 20 characters and must not contain empty spaces");
        }
        else if (password !== confirmedPassword) {
            addInputWarningMessage("Password and confirmed password are not the same one");
        }
            ...
            if (response.ok) {
                ...
                if (response.status == 200) {
                    if (inputWarningMessage) {
                        ...
                        inputWarningMessage.classList.add("success");
                        if (userPfp instanceof HTMLInputElement &&
                            userPfp.files &&
                        ...
                        async function uploadPfp(fileInput) {
