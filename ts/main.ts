...
const userOnlineSpan = document.querySelector(
  ".chat-header .header-container-text-chat span"
);
...
const pfpImage = document.querySelector<HTMLImageElement>(
  ".sidebar div .profile-picture"
);
const chatUserOptions = document.querySelector(
  ".chat .chat-header .container-icon-tools #user-options"
);
const chatUserOptionsContainer = document.querySelector(
  ".chat .chat-header .container-icon-tools #user-options .options-user-container"
);
const mainUserOptions = document.querySelector(".main-user-options .icon-dots");
const menuUserOptions = document.querySelector(
  ".main-user-options .menu-user-options"
);
const menuUserOptionsSignOut = document.querySelector(
  ".main-user-options .menu-user-options .sign-out-btn"
);
const deleteAccountBtn = document.querySelector(
  ".main-user-options .menu-user-options .delete-account-btn"
);
let mainUserNameTextContent = "";
let mainUserDisplayNameTextContent = "";
let mainUserBioTextContent = "";
const formMessage = document.querySelector<HTMLFormElement>(".chat form");
const inputSendMessage = document.querySelector<HTMLTextAreaElement>(
  '.input-container textarea[name="name"]'
);
...
const setUsersId: Set<number> = new Set();
let userId = -1;
let chatGuestId = -1;
let chatId = -1;
...
ws.onmessage = (event) => {
  ...
  if (messageJson && messageJson.type === "init") {
    ...
    if (
      !Array.from(setUsersId).some((userChatId) => {
    ...
    if (chatId === chatIdRes) {
      const messageContainerDivs = document.querySelectorAll(
    ...
    let messagesContainers = document.querySelectorAll(
    ...
    for (let i = 0; i < messagesContainers.length; i++) {
      ...
      if (messageId === Number(messageIdDiv)) {
        ...
        if (
          div.querySelector(
            ".message-pfp-container .message-user-profile-picture"
          )
        ) {
          const chatUserPfpUrl = document
            .querySelector<HTMLImageElement>(".chat .chat-header .picture-chat")
            ?.getAttribute("src");

          const previousDivMessage = messagesContainers[i + 1];
          const pfpContainerDiv = previousDivMessage?.querySelector(
            ".message-pfp-container"
          );

          if (
            previousDivMessage?.classList.contains("message-other") &&
            pfpContainerDiv
          ) {
    ...
    messagesContainers.forEach((div) => {
      const messageTextSpan = div.querySelector(
        ".messageUpperContainer .span-message .text-content"
      );
      const editedLabelText = div.querySelector(
        ".messageLowerContainer .edited-label-message"
      );
      let messageIdDiv = div.getAttribute("id");
...
async function checkUserSessionKey() {
  try {
    const response = await fetch(
      "https://restfulapi-chatapp.onrender.com/session/validate",
      {
        method: "POST",
        headers: {
    ...
    const responseBody: User = JSON.parse(jsonRes);
    if (response.ok) {
...
const searchUserInput =
  document.querySelector<HTMLInputElement>("#search-user");
...
function updateMainUserData(
  ...
  if (pfp != null && pfp.type === "image") {
    document
      .querySelector<HTMLImageElement>(
        ".menu-profile .container-modal label .menu-profile-picture"
      )
  ...
  let mainUserName = document.querySelector<HTMLTextAreaElement>(
    ".menu-profile .main-user-name"
  );
  if (name.length > 0) {
    if (mainUserName) mainUserName.value = name;
    mainUserNameTextContent = name;
  }
  let mainUserDisplayName = document.querySelector<HTMLTextAreaElement>(
    ".menu-profile .main-user-display-name"
  );
  if (display_name.length > 0) {
    if (mainUserDisplayName) mainUserDisplayName.value = display_name;
    mainUserDisplayNameTextContent = display_name;
  } else if (name.length > 0) {
    if (mainUserDisplayName) mainUserDisplayName.value = name;
    mainUserDisplayNameTextContent = name;
  }
  let mainUserBio = document.querySelector<HTMLTextAreaElement>(
    ".menu-profile .main-user-bio"
  );
  if (bio != null) {
    if (bio.length > 0) {
      if (mainUserBio) mainUserBio.value = bio;
      mainUserBioTextContent = bio;
    }
  }
}
async function uploadPfpImage(fileInput: HTMLInputElement) {
...
async function searchUser(query: string) {
  try {
    const response = await fetch(
      "https://restfulapi-chatapp.onrender.com/user/search",
      {
        method: "POST",
        headers: {
          "Content-Type": "text/plain",
...
function createUsersElements(users: User[], previous: boolean = false) {
  ...
  users.forEach((user: User) => {
    ...
    if (user.pfp && user.pfp.type === "image") {
      pfp.setAttribute(
        "src",
        user.pfp.content ?? "../img/default-avatar-profile-icon-social.webp"
      );
...
function createChat(
  ...
  const textareaBar = document.querySelector<HTMLTextAreaElement>(
    '.chat form textarea[name="name"]'
  );
  const userName = chatHeader?.querySelector(".header-container-text-chat h1");
  const blockUserOption = document.querySelector(
    ".chat .chat-header .container-icon-tools #user-options .options-user-container #block-user-option"
  );
  const deleteChatOption = document.querySelector(
    ".chat .chat-header .container-icon-tools #user-options .options-user-container #delete-user-option"
  );
  ...
  function createMenuBlockUser() {
    ...
    blockUserForm?.addEventListener("submit", async (event) => {
      ...
      if (userId > 0 && id > 0 && checkboxBlockUser && checkboxHideChat) {
        try {
          const response = await fetch(
            "https://restfulapi-chatapp.onrender.com/user/block",
            {
              method: "POST",
              headers: {
  ...
  function createMenuDeleteChat() {
    ...
    btnConfirm.addEventListener("click", async () => {
      if (userId > 0 && id > 0) {
        try {
          const response = await fetch(
            "https://restfulapi-chatapp.onrender.com/chats/delete",
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            if (response.status === 200) {
              ...
              if (menuDeleteChat) menuDeleteChat.remove();
              deleteChatOption?.removeEventListener(
                "click",
                createMenuDeleteChat
              );
              blockUserOption?.removeEventListener(
                "click",
                createMenuBlockUser
              );
              document
                ...
                .forEach((div) =>
                  Number(div.id) === id ? div.remove() : null
                );
            } else {
              //console.log(responseBody);
              if (menuDeleteChat) menuDeleteChat.remove();
  ...
  async function createUserInfo() {
    ...
    try {
      const response = await fetch(
        "https://restfulapi-chatapp.onrender.com/user/profileInfo",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
function createMessageElements(
  ...
  if (
    typeof responseBody[i].content === "string" &&
  ...
  if (responseBody[i].sender_id === userId) {
    ...
    if (responseBody[i].sender_id !== responseBody[i + 1]?.sender_id) {
      const lastDivMessage =
        document.querySelector(".chat .messages")?.firstElementChild;
      const chatUserImg = document.createElement("img");
      chatUserImg.classList.add("message-user-profile-picture");
      chatUserImg.setAttribute("src", chatUserPfpUrl ?? "");
      chatUserImg.setAttribute("alt", "profile picture");
...
async function getChat(ownerId: number, guestId: number) {
  const usersId = {
    ownerId: ownerId,
    guestId: guestId,
  };
  try {
    const response = await fetch(
      "https://restfulapi-chatapp.onrender.com/chats/get",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      if (response.status == 200) {
        const responseBody: ChatResponse[] = await response.json();
        const message: WsReq = {
          type: "seen",
          senderId: guestId,
...
async function getPreviousChats() {
  if (userId > 0) {
    ...
    try {
      const response = await fetch(
        "https://restfulapi-chatapp.onrender.com/chats/getPrevious",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        if (response.status === 200) {
          //console.log("Response body:", responseBody);
          let users: User[] = JSON.parse(responseBody);
          //console.log(users);
          //console.log(response.status);
...
async function dynamicFetchMessages(
  messageDateSent: string,
  senderId: number,
  guestId: number
) {
  try {
    if (chatId > 0) {
      const response = await fetch(
        "https://restfulapi-chatapp.onrender.com/messages/getNew",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
async function sendMessage(
  ...
  if (
    ...
    try {
      const response = await fetch(
        "https://restfulapi-chatapp.onrender.com/message/send",
        {
          method: "POST",
          body: formData,
        }
      ...
      if (response.ok) {
        if (response.status === 200) {
          ...
          const messageRes: ChatResponse[] = [responseBody];
...
async function deleteMessage(
  ...
  if (messageId > 0) {
    try {
      const response = await fetch(
        "https://restfulapi-chatapp.onrender.com/message/delete",
        {
          method: "DELETE",
          headers: {
            "Content-Type": "text/plain",
...
async function editMessage(
  ...
  if (messageId > 0) {
    const messageSpan = divContainerMessage.querySelector(
      ".messageUpperContainer .span-message"
    );
    const messageTextSpan = divContainerMessage.querySelector(
      ".messageUpperContainer .span-message .text-content"
    );
    if (messageTextSpan) {
      ...
      textareaEditText.addEventListener("keydown", async (event) => {
        if (event.key === "Enter" && !event.shiftKey) {
          ...
          if (editedText === messageText && messageTextSpan) {
            ...
            try {
              const response = await fetch(
                "https://restfulapi-chatapp.onrender.com/message/edit",
                {
                  method: "PATCH",
                  headers: {
                    "Content-Type": "application/json",
              ...
              if (response.ok) {
                ...
                if (response.status === 200) {
                  ...
                  if (refMessageTextSpan) {
                    ...
                    const textEditedLabel = divContainerMessage.querySelector(
                      ".edited-label-message"
                    );
                    if (textEditedLabel) {
...
pfpImage?.addEventListener("click", () => {
  if (document.querySelector(".menu-profile")) {
    ...
    pfpUpload?.addEventListener("change", function (this: HTMLInputElement) {
      if (
    ...
    userInfoForm?.addEventListener("submit", async (event) => {
      ...
      if (
        ...
        try {
          ...
          const response = await fetch(
            "https://restfulapi-chatapp.onrender.com/user/editProfile",
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
          ...
          const responseBody: User = JSON.parse(resText);
          if (response.ok) {
            if (response.status === 200) {
              //console.log(responseBody);
              updateMainUserData(
  ...
  let optionsMenu = document.querySelectorAll<HTMLDivElement>("#optionsMenu");
...
deleteAccountBtn?.addEventListener("click", () => {
  ...
  btnConfirm.addEventListener("click", async () => {
    if (userId > 0) {
      try {
        const response = await fetch(
          "https://restfulapi-chatapp.onrender.com/user/delete",
          {
            method: "DELETE",
            headers: {
              "Content-Type": "text/plain",
